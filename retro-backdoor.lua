...
surface.CreateFont( "tabButtons", { font = "Astriumtabs2", extended = false, size = 65, weight = 400})
surface.CreateFont( "labels", { font = "Calibri", extended = false, size = 16, weight = 500})
...
while util.NetworkIDToString(i) do
    if util.NetworkIDToString(i) == str then
        if table.HasValue(Chnl, util.NetworkIDToString(i - 1)) || table.HasValue(Chnl, util.NetworkIDToString(i + 1)) then
                return true
            end
        end
        i = i + 1
...
draw.LinearGradient(qrex.x + 10, qrex.y + 10, parent:GetWide() - 11, 2, { {offset = 0, color = Color(59, 175, 222)}, {offset = 0.5, color = Color(202, 70, 205)}, {offset = 1, color = Color(201, 227, 58)} }, true)
...
qrexPanel = vgui.Create( "DPanel" )
    if(panelTable[1]) then qrexPanel:SetVisible(false) end
...
hook.Add("Think", "keyToggle", function() 
    if input.IsKeyDown(openKey) && !toggle then
        toggle = true
